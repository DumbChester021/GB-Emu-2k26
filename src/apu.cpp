...
void APU::powerOff() {
    ...
    ch1_ = PulseChannel{};
    ch2_ = PulseChannel{};
    // Preserve wave RAM on power off (DMG behavior)
...
uint8_t APU::readReg(uint16_t addr) const {
    ...
    if (addr >= 0xFF30 && addr <= 0xFF3F) {
        if (ch3_.enabled) {
            // When CH3 is active, reads return the byte currently being read
            // by the wave channel (on DMG, this is somewhat unpredictable)
...
void APU::writeReg(uint16_t addr, uint8_t val) {
    ...
    if (addr >= 0xFF30 && addr <= 0xFF3F) {
        if (ch3_.enabled) {
            // When CH3 is active on DMG, writes go to the byte being read
    ...
    if (!powered_) {
        ...
        switch (addr) {
            case 0xFF11: ch1_.dutyLength = val; ch1_.lengthCounter = 64 - (val & 0x3F); return;
            case 0xFF16: ch2_.dutyLength = val; ch2_.lengthCounter = 64 - (val & 0x3F); return;
            case 0xFF1B: ch3_.length = val; ch3_.lengthCounter = 256 - val; return;
            case 0xFF20: ch4_.length = val; ch4_.lengthCounter = 64 - (val & 0x3F); return;
            default: return;
        }
    }
    ...
    switch (addr) {
        ...
        case 0xFF14: // NR14 — Trigger + Length Enable + Freq High
            ch1_.freqHi = val;
            // Extra length clocking on frame sequencer edge case
            if ((val & 0x40) && !(frameSequencerStep_ & 1)
                && ch1_.lengthCounter > 0) {
                ch1_.lengthCounter--;
